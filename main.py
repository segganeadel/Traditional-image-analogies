...
A_path = glob(os.path.join('imgs', folder, 'A.*'))[0]
Ap_path = glob(os.path.join('imgs', folder, 'Ap.*'))[0]
...
B_paths = natsorted(glob(os.path.join('imgs', folder, "B", 'B*.*')))
...
output_folder = os.path.join('imgs', folder, 'out')
...
for i, (B_L, Bp_L, s) in enumerate(zip(B_L_list, Bp_L_list, s_list)):
    ...
    cv2.imwrite(os.path.join(output_folder, write_name),imgBp)
